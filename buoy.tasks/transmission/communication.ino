...
unsigned short int remote_msg(char **buf, unsigned int *buf_size, unsigned int *buf_allocated, char **answer, unsigned int *answer_size, unsigned int *answer_allocated) {
    ...
    // Put all together
    strcat_realloc(&transmission_config.mavlink_buf, &transmission_config.mavlink_buf_size, &transmission_config.mavlink_buf_allocated, *buf, *buf_size, __FILE__, __LINE__);
    // print_ashex(transmission_config.mavlink_buf, transmission_config.mavlink_buf_size);

    // While data in the buffer and not MAVLINK message detected, keep reading
    yield();
    while (buf_idx<transmission_config.mavlink_buf_size) {

        // Parse MAVLINK mg
        if (mavlink_parse_char(chan, (uint8_t) transmission_config.mavlink_buf[buf_idx], &msg, &status)) {
...
*/
    ...
    (*buf_size) = 0;
    if (bypass) {
        strcat_realloc(buf, buf_size, buf_allocated, transmission_config.mavlink_buf, buf_idx, __FILE__, __LINE__);
    }

    // Move back what is left to the beginning of the buffer
    if (buf_idx<transmission_config.mavlink_buf_size) {
        // Left some bytes for reading in the future
        memmove(transmission_config.mavlink_buf, transmission_config.mavlink_buf+buf_idx,transmission_config.mavlink_buf_size-buf_idx);
    } else {
        // Everything readen
    ...
    if (transmission_config.mavlink_buf_size) {
        print_debug(CCOMMUNICATION, stdout, CBLUE, 0, "MAVLINK Buffer: %d", transmission_config.mavlink_buf_size);
    }
