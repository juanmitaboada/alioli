...
unsigned short int strcat_realloc(char **target, size_t *targetsize, size_t *allocated, const char *source, size_t sourcesize, const char *file, unsigned int line) {
    ...
    if (temprealloc) {
        ...
        *targetsize += sourcesize;
        ...
        // No memory available
        print_debug(ALIOLI, stderr, CRED, 0, "%s:%d: ERROR no memory available to strcat_realloc()! (called from %s:%d)", __FILE__, __LINE__, file, line);
        // Error happened
        error = 1;
    }
    return !error;
...
void print_debug(const char *mainname, FILE *output, unsigned short int color, unsigned short int modifier, const char *fmt, ...) {
...
#if RTC
    DateTime datetime;
    unsigned short int use_rtc = 1;
#endif

    // Set modifier (convert to normal)
    if (modifier>=10) {
        modifier -= 10;
#if RTC
        use_rtc = 0;
    ...
    if (output) {
        ...
        fprintf(output, tail);
        if (!nocolor) {
            fprintf(output, COLOR_CLOSE);
        }
        fflush(output);
    } else {
        // Otherwise use normal output
        if (!nocolor) {
            Serial.print(pcolor);
        }
        Serial.print(header);
